#!/usr/bin/env python3
"""
使用 gh api graphql 批量添加项目到 Lists。
避免 requests 的 SSL 问题。
"""
...
def batch_mutate(items, max_retries=3):
    """
    ...
    for attempt in range(max_retries):
        ...
        if result.returncode != 0:
            ...
            if attempt < max_retries - 1:
                ...
                continue
            return {alias: (False, err[:100]) for alias, _, _ in items}
        ...
        try:
            ...
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)
                continue
            return {alias: (False, "JSON parse error") for alias, _, _ in items}

        if "errors" in data and len(data["errors"]) == len(items):
            # 全部失败，可能是请求级错误
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)
                continue
...
def main():
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        results = batch_mutate(batch_items)
